...
class Game extends React.Component {
  ...
  draw = () => {
    ...
    for (let i = 0; i < this.state.pipe.length; i++) {
      ...
      const nerdTouchedPipeSide =
        nerdX + nerd.width >= this.state.pipe[i].x && nerdX <= this.state.pipe[i].x + pipeUp.width;
      const nerdTouchedPipeUpBottom = nerdY <= this.state.pipe[i].y + pipeUp.height;
      const nerdTouchedPipeDownBottom =
        nerdY + nerd.height >= this.state.pipe[i].y + pipeUp.height + gap;
      ...
      if ((nerdTouchedPipeSide && nerdTouchedPipeBottoms) || nerdTouchedCanvasBottom) {
        ...
        setTimeout(() => {
          this.redSreen.style.display = 'none';
        }, 200);
    ...
    this.setState({
      pipe: this.state.pipe.map(object => ({
        x: object.x - 4,
        y: object.y
      }))
    });
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="playerName" style={{ color: colors }}>
          {playerName}
        </div>
        <div className="score" style={{ color: colors }}>
          Score: {this.state.score}
        </div>
        <div className="redScreen" ref={ref => (this.redSreen = ref)} />
      </div>
    );
  }
}
